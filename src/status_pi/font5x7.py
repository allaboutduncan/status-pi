"""Classic 5x7 dot-matrix font (column-major, LSB = top row).

One entry per printable ASCII character, five columns each.  This is the
long-serving GLCD/Adafruit-GFX bitmap: it is the right shape for an LED
matrix, and shipping it inline means no font files and no licence headaches
on the device.
"""

FONT_WIDTH = 5
FONT_ROWS = 8  # glyph cells are 8 tall; uppercase only needs the top 7
...
# A few characters we are likely to meet in meeting titles but cannot draw.
_TRANSLITERATE = {
    "–": "-", "—": "-", "‘": "'", "’": "'",
    "“": '"', "”": '"', "…": "...", " ": " ",
    "•": "*", "·": "-",
...
def normalise(text: str) -> str:
    """Map smart punctuation onto glyphs we actually have."""
    return "".join(_TRANSLITERATE.get(ch, ch) for ch in text)


def glyph(char: str):
    """Five column bitmaps for `char`, falling back to '?'."""
    return _GLYPHS.get(char, _FALLBACK)
